...
/**
 * This annotation is used to identify PlayerController class as RestController
 * which is responsible for handling HTTP requests related to players.
 */
@RestController

/**
 * This annotation at the class level specifies the base URL path for all the
 * end points in this controller. All end points in this controller will be
 * under the '/players' URL path.
 */
...
public class PlayerController {
	...
	// getAllPlayers() is mapped to the HTTP Get request for the '/getAllPlayers'
	// end point.
	...
	public ResponseEntity<Object> getAllPlayers() {
		...
		/*
		 * Checks if players were found and returns the ResponseEntity object
		 * accordingly. If found, it creates a ResponseEntity with HTTP status code of
		 * 302(found), and sets the body to return the list of players. If no players
		 * found, it creates RE with HTTP status code of 404(not found), and sets a
		 * message in the body.
		 */
	...
	// getPlayerById() is mapped to the HTTP Get request for the '/getPlayerById'
	...
	public ResponseEntity<Object> getPlayerById(@PathVariable Long playerId) {
		...
		/**
		 * Checks if the player is found. If yes, returns the player with HTTP status
		 * code Found. If no, returns a message with HTTP status code Not_Found.
		 */
	...
	// getPlayersByTeam() is mapped to the HTTP Get request for the
	// '/getPlayersByTeam' using @PathVariable to capture
	...
	public ResponseEntity<Object> getPlayersByTeam(@PathVariable String teamName) {
		...
		/**
		 * Checks if players are found. If yes, returns the list with HTTP status code
		 * Found. If no, returns a message with HTTP status code Not_Found.
		 */
		return players != null ? ResponseEntity.status(HttpStatus.FOUND).body(players)
		...
		if (player != null) {
			...
			if (playerService.playerExists(player)) {
				return ResponseEntity.status(HttpStatus.BAD_REQUEST)
						.body("Player with this name and jersey number already exists!");
			}
	...
	public ResponseEntity<Object> createManyPlayers(@RequestBody(required = false) List<Player> players) {
		...
		if (players != null) {
			//checking if any player already exists
			Player existingPlayer = playerService.existingPlayerFromList(players);
			if (existingPlayer != null) {
				return ResponseEntity.status(HttpStatus.BAD_REQUEST)
						.body("Player with name " + existingPlayer.getPlayerName() + " and jersey number "
								+ existingPlayer.getJerseyNumber() + " already exists!");
			}
	...
	// updatePlayer() is mapped to the HTTP Put request for the '/updatePlayer'
	// using @PathVariable to capture id and @RequestBody Player object.
	...
	public ResponseEntity<Object> updatePlayer(@PathVariable Long playerId, @RequestBody Player updatePlayer) {
		 ...
		 * If existingPlayer not found, a message is displayed with HTTP status code
		 * Not_Found.
		 */
	...
	// deletePlayerById() is mapped to the HTTP Delete request for the
	// '/deletePlayerById' using @PathVariable to capture id.
	@DeleteMapping({ "/deletePlayerById/{playerId}" })
	public ResponseEntity<Object> deletePlayerById(@PathVariable Long playerId) {
		// Deletes a specified player and returns true if successful, or false
		// otherwise.
		Boolean isDeleted = playerService.deletePlayerById(playerId);
		/**
		 * If deletion is successful, returns a message and HTTP status code OK. If not,
		 * returns a message and HTTP status code Bad_Request.
		 */
	...
	// deletePlayersByTeam() is mapped to the HTTP Delete request for the
	// '/deletePlayersByTeam' using @PathVariable to capture the team name.
	@DeleteMapping("/deletePlayersByTeam/{teamName}")
	public ResponseEntity<Object> deletePlayersByTeam(@PathVariable String teamName) {
		// Delete players belongs to a specified team and returns true if successful, or
		// false otherwise.
	...
	// deleteAllPlayers() is mapped to the HTTP Delete request for the
	// '/deleteAllPlayers'
 ...
 //--------------------new functionality countPlayersInATeam() function is used here------------------//
	...
	//countAllPlayers is mapped to the HTTP Get request for the '/countAllPlayers'
    @GetMapping("/countAllPlayers")
    public ResponseEntity<Object> countAllPlayers(){
    	//count all players in the database
    	long playersCount = playerService.countAllPlayers();
    	/**
    	 *if count is not zero, it will return OK status,
    	 *otherwise No_Content
    	 */
    	return playersCount != 0 ? ResponseEntity.status(HttpStatus.OK).body("There are " + playersCount + " players in the database.") :
    		ResponseEntity.status(HttpStatus.BAD_REQUEST).body("No player exists!");
    }
    ...
    public ResponseEntity<Object> countPlayersInATeam(@PathVariable String teamName){
    	...
    	return playersCount != 0 ? ResponseEntity.status(HttpStatus.OK).body("Team " + teamName + " has " + playersCount + " players.") :
    		ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Team " + teamName + " doesn't exists!");
    }
