...
public class PlayerService {
	...
	//to ensure the consistency and integrity of our database during update and delete operations
	@Transactional
	public Player updatePlayer(Long playerId, Player updatedPlayer) {
		...
		if (existingPlayer != null) {
			...
			// Checks and updates the existing player's total Assists if the updated player
			// has
	...
	// ---------------------------new functionality assignBadges()
	// implemented------------------------------------//
	/**
	...
	// ---------------------new functionality countAllPlayers()
	// function-----------//
	/**
	 * defines a method that returns a long value representing the count of players
	...
	public Player getTopGoalScorer(List<Player> players) {
		...
		for (Player player : players) {
			if (player != null) {
				int playerTotalGoals = (player.getTotalGoals() != null) ? player.getTotalGoals() : 0;
