...
public class CrossCountryRace {
    ...
    public static void main(String[] args) {
        ...
        while(true) {
            ...
            ScoringStrategy strategy = chooseScoringStrategy(sc);
            
            for (int i = 0; i < inputs.length; i++) {
            	String teamName = inputs[i];
                teamMap.computeIfAbsent(teamName, k -> new ArrayList<>())
                .add(new Runner(i + 1, new Team(teamName)));
            }

            // Converting team runner lists to a list of Team objects.
            List<Team> teams = new ArrayList<>();
            for (Map.Entry<String, List<Runner>> entry : teamMap.entrySet()) {
                teams.add(new Team(entry.getValue(), strategy));
            }
    ...
    private static ScoringStrategy chooseScoringStrategy(Scanner sc) {
    	while (true) {
            System.out.println("Choose a scoring strategy: ");
            System.out.println("1. Standard Scoring");
            System.out.println("2. Bonus Scoring");
            ...
            switch (choice) {
                ...
                case "2":
                    return new BonusScoringStrategy();
                default:
